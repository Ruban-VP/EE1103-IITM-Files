/* ***   Downsampling and Interpolation using Cubic spline with LU decomposition (Error printer) ***
...
a cubic spline (assisted by LU decomposition) and expands it back to the original, non-interpolated datapoints. Finally, it calculates the mean squared error
between the original output and the interpolated output values and prints it. 

Created by: V. Ruban Vishnu Pandian EE19B138
Created on: 30th September, 2020

Command input example: ./a.out <Downsampling constant> <File path> (Used after the linux command gcc LUD_Cubspline_Errorvsg.c -lm) (or)
        ./<Filename> <Downsampling constant> <File path> (<Filename> is the name used in linux command "gcc LUD_Cubspline_Errorvsg.c -lm -o <Filename>")
*/
...
int main(int argc, char** argv)  // Opening of the main function and getting the command line arguments
{
    ...
    else if(f==g && g>0)      // Checks if the downsampling constant entered is a positive integer
    {
        ...
        if(rows>=g)
        {
            ...
            double meansqerror = 0;           // Variable that stores mean squared error
            for(int i=0; i<rows; i++)
            {
            	meansqerror += pow((y[i]-intpol[i]),2);   // Updating the error variable as:
                                                          // New error = previous error + square of difference of original and interpolated outputs
            }
            meansqerror /= rows;               // For getting the mean error

            printf("%d %le\n", g, meansqerror);     // Printing the mean squared error
